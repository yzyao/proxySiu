...
待检测代理存在时，每批只处理 Pending，并在上一批完成后立即安排下一批；自动采集会暂缓到待检缓冲清空。采集入池的非固定 Pending 数也受档位缓冲上限约束（`high-throughput` 为 1,200，`idc-safe` 为 300），超出的候选会进入 24 小时重入隔离，避免候选总数填满池容量。待检缓冲清空后，稳定阶段先给 Alive、Dead 保留本批检测名额并按到期时间补齐。右上角“检测到期代理”只处理已经到期的记录；下方“强制检测一批”会忽略到期时间，但仍遵守本批上限和状态优先级。
...
# 返回最多 20 条低延迟的可用 HTTP 代理（JSON 数组）
curl -H "Authorization: Bearer $PROXYSIU_ACCESS_TOKEN" \
  'https://proxy.example.com/api/proxy/random?protocol=http&count=20'
